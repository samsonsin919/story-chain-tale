...
/**
 * Internal: AI-extract proper nouns from a story and upsert them into the
 * dictionary. No auth check — only call from trusted server contexts.
 */
export async function extractTermsImpl(storyId: string, upToPosition: number) {
  const [{ data: story }, { data: segs }] = await Promise.all([
    supabaseAdmin.from("stories").select("title, opening").eq("id", storyId).single(),
    supabaseAdmin
      .from("story_segments")
      .select("position, content")
      .eq("story_id", storyId)
      .order("position", { ascending: true })
      .lte("position", upToPosition),
  ]);
  if (!story) return { terms: [] as Array<{ term: string; kind: string }> };
